...
class FileSystemEntity:
    ...
    entity_type – The type of the entity ('root', 'drive', 'folder', 'byte', 'text').
    ...
    size – an integer defined as follows:
        ...
        For a byte file, it is one half of the sum of all sizes of the entities it contains.
    """

    VALID_ENTITIES = ['root', 'drive', 'folder', 'byte', 'text']
    ...
    def entity_type(self, new_type):
        if new_type not in self.VALID_ENTITIES:
            raise IllegalFileSystemOperation(
                'Not a valid entity type, must be \'drive\', \'folder\', \'byte\', or \'text\''
            )
        else:
...
class Drive(Container):
    ...
    VALID_DRIVE_CHILDREN = ['folder', 'byte', 'text']
...
class Folder(Container):
    """
    Represents a folder, which can be contained within drives, folders, and byte
    and can contain folders, byte, and text.
    """

    VALID_FOLDER_CHILDREN = ['folder', 'byte', 'text']
...
class byte(Container):
    ...
    VALID_BYTE_CHILDREN = ['folder', 'byte', 'text']
    ...
    def add_child(self, child):
        if child.name in self._children.keys():
            raise PathAlreadyExists('An entity with that name already exists')
        elif child.entity_type not in self.VALID_BYTE_CHILDREN:
            raise IllegalFileSystemOperation('Entity is not valid for adding to a byte')
        else:
            self._children[child.name] = child
